...
class CNN_regression(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(64, 3)                                                                 # 출력 레이어 (64 -> 8, 8개의 클래스 예측) 
    def forward(self, x):
        x = x.permute(0, 2, 1)                  
...
def train_cnn_regression(config, X_train, X_valid, y_train, y_valid):
    ...
    save_model(model, './CNN_regression/model/cnn_regression_model.pth')
    return model
...
class CNN_LITE_regression(nn.Module):
    def __init__(self):
        super(CNN_LITE_regression, self).__init__()
        # 1D Convolutional Layers
        ...
        self.fc3 = nn.Linear(16, 3)                                                                 # 출력 레이어 (16 -> 8, 8개의 클래스 예측)
    def forward(self, x):       
        x = x.permute(0, 2, 1)                                            
        x = torch.relu(self.conv1(x))
        x = torch.relu(self.conv2(x))
        ...
        x = self.fc3(x)
        return x
def train_cnn_lite_regression(config, X_train, X_valid, y_train, y_valid):
    model = CNN_LITE_regression()
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=config['learning_rate'])
    ...
    save_model(model, './CNN_regression/model/cnn_lite_regression_model.pth')
    return model